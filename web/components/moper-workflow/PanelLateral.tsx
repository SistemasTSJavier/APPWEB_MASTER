...
export function PanelLateral({ registroIdActual, onSeleccionarRegistro, onNuevoRegistro, refreshTrigger = 0 }: PanelLateralProps) {
  const { authHeaders, user, puedeCancelar } = useMoperWorkflow()
  ...
  const puedeCancelarEmail =
    puedeCancelar ||
    (!!user?.email && EMAILS_PUEDEN_CANCELAR.includes(user.email.trim().toLowerCase()))
  ...
  const cancelarRegistro = useCallback((id: number, e: React.MouseEvent) => {
    e.stopPropagation()
    if (!puedeCancelarEmail) return
    setMenuAbiertoId(null)
    setCancelandoId(id)
    moperFetch(`/api/moper/${id}/cancelar`, {
      method: 'PATCH',
      headers: authHeaders(),
    })
  ...
  }, [authHeaders, puedeCancelarEmail])

  const cargar = () => {
    setCargando(true)
    setErrorApi(null)
    moperFetch(`/api/moper`, { headers: authHeaders() })
      .then((r) => r.json().then((d) => ({ ok: r.ok, data: d })))
      .then(({ ok, data }) => {
