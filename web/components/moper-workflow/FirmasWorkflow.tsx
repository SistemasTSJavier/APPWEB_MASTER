...
export function FirmasWorkflow({ registroId, registro, onFirmaRegistrada, modoCodigo = false, codigoAcceso: codigoAccesoProp = '', firmaDestacada = null }: FirmasWorkflowProps) {
  ...
  const registrarFirma = async (tipo: string, imagen: string, codigo?: string) => {
    ...
    try {
      ...
      const res = await moperFetch(`/api/moper/${registroId}/firma`, {
        method: 'PATCH',
          ...
          return (
            ...
            >
              ...
              {firmado ? (
                ...
                <p className="text-sm text-oxford-600">Solo el oficial puede firmar entrando con el codigo de acceso.</p>
              ) : (key === 'rh' && !puedeFirmarRh) || (key === 'gerente' && !puedeFirmarGerente) || (key === 'control' && !puedeFirmarControl) ? (
                <p className="text-sm text-oxford-500">Sin permiso para esta firma.</p>
              ) : (
                <button
                  type="button"
